...
describe.each([false, true])('Float (optimize: %s)', optimize => {
  ...
  describe('FloatStringValidator', () => {
    it.skip('should generate validation code and give same result', () => {
      const validator = new FloatStringValidator(1, 2, { optimize })
    ...
    it('requires value to be a float', function() {
      ...
      expect(validator.validate('')).toStrictEqual([
        new WrongLengthFail('Must be an float or a string with an float (received "")')
      ])
      expect(validator.validate('a')).toStrictEqual([
        new NotFloatStringFail('Must be an float or a string with an float (received "a")')
      ])
      expect(validator.validate({})).toStrictEqual([
        new NotFloatStringFail('Must be an float or a string with an float (received "[object Object]")')
      ])
      expect(validator.validate([])).toStrictEqual([
        new NotFloatStringFail('Must be an float or a string with an float (received "")')
      ])
      expect(validator.validate(true)).toStrictEqual([
        new NotFloatStringFail('Must be an float or a string with an float (received "true")')
      ])
      expect(validator.validate(false)).toStrictEqual([
        new NotFloatStringFail('Must be an float or a string with an float (received "false")')
      ])
    })
  ...
  describe('FloatString', () => {
    it('accepts empty value', () => {
      const validator = FloatString(0, Number.MAX_SAFE_INTEGER, false)
      expect(validator.validate(null)).toStrictEqual([])
      expect(validator.validate(undefined)).toStrictEqual([])
    })

    it('rejects empty value', () => {
      const validator = FloatString(0, Number.MAX_SAFE_INTEGER)
